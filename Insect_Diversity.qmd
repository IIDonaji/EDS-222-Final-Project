...
# Load Data
insect <- read_csv(here("data/insect-raw-data.csv"))
#view(insect)
```

## Background Information

Urban areas are expanding globally, yet we know little about how landscaping choices affect urban biodiversity. Los Angeles, a naturally arid Mediterranean climate, presents a unique opportunity to examine whether drought-tolerant landscaping supports insect diversity. 
...
A conceptual model representing the hypothesized causal relationships among variables:
...
# Data explartion and cleaning

```{r, include = FALSE}
# Data Exploration
str(insect)
skim(insect)
colSums(is.na(insect))
        
```

```{r,include = FALSE}
# Summary of Key Response Variables 
summary(insect[,c("Richness", "Abundance", "CorRich", "CorAbun")])


# Distribution Plots
par(mfrow=c(2,2))
hist(insect$Richness, main="Richness Distribution", xlab="Species Richness")
hist(insect$Abundance, main="Abundance Distribution", xlab="Abundance")
hist(insect$CorRich, main="Corrected Richness", xlab="CorRich")
hist(insect$CorAbun, main="Corrected Abundance", xlab="CorAbun")
```
...
### Before Modeling:
**Site-level aggregation:** To avoid pseudoreplication from repeated monthly sampling, I averaged species richness and temperature across the year for each site, yielding 30 independent observations.
...
print(combined)
ggsave(here("figures","combined_exploratory.png"), combined, width = 12, height = 9, dpi = 300)
```
...
To demonstrate the model is correctly specified, I first simulated data with known parameter values, then fit our model to recover those parameters.
...
kable(comparison, digits = 3, # 3 decimal places
...
main_plot <- ggplot(pred_df, aes(x = mean_temp, y = pred, color = Drought, fill = Drought)) +
...
ggsave(here("figures","main_plot.png"), main_plot, width = 12, height = 9, dpi = 300)
```
...
simple_plot <- ggplot(pred_summary, aes(x = mean_temp, y = pred_mean, 
...
ggsave(here("figures","simple_plot.png"), simple_plot, width = 12, height = 9, dpi = 300)
...
ci_rate_ratio <- exp(confint(richness_model)["DroughtYes",])
...
Using negative binomial regression on site-aggregated data, I found that drought-tolerant plants increase insect species richness by approximately 50% in urban Los Angeles. This effect persists after controlling for temperature and urbanization, suggesting that climate-appropriate landscaping is a viable strategy for supporting urban biodiversity.
